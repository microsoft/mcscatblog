...
Let's think about presenting information retrieved from tools.  Copilot Studio offers up the ability to present [tool results as an adaptive card](https://learn.microsoft.com/en-us/microsoft-copilot-studio/advanced-plugin-actions#completion), but what about when we want to combine information from multiple tools?

For example, say I have an agent who helps me plan journeys. The agent has tools to find route information, which may vary wildly depending on the mode of transport:
...
However, if I ask for a different route, I get different information for a different mode of transport, and the weather information is not included:

![Train Journey](/assets/posts/adaptive-card-generation/train-journey.png){: .shadow w="487" h="764"}

So how does it work? For this example, I wanted to rely on generative orchestration as much as possible, so I implemented a topic which fires on OnGeneratedResponse. This gives me access to System.Response.FormattedText (the message about to be sent to the user) and allows me to decide whether I want to intercept and change the message.
...
```
The prompt looks like this.  Essentially it just says "figure out the best form description to use, and use it to generate the JSON":
...
2. As the response is formed, the OnGeneratedResponse event fires and triggers my topic
3. My topic passes the response and scenario/formdescription object into the custom prompt
...
[(...worth a read if you haven't)](https://en.wikipedia.org/wiki/Don%27t_Make_Me_Think)
...
Everything was done on the fly; both the form generation and the interpretation of the user's response to it.
